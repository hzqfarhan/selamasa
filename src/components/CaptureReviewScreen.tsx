...
function ShareButton({ caption, onShare }: { caption: string; onShare: (caption: string) => Promise<void> | void }) {
  ...
  return (
    <button
      onClick={handleClick}
      disabled={uploading}
      style={{ padding: '14px', background: uploading ? 'rgba(74, 144, 226, 0.5)' : 'linear-gradient(135deg, var(--gold), #1f4a7c)', borderRadius: '12px', color: '#fff', fontSize: '14px', fontWeight: 'bold', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
    >
      {uploading ? (
        <>
