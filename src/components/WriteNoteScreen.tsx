...
export default function WriteNoteScreen({ coupleName, guestName, onClose, onSend, onVoiceRedirect }: WriteNoteProps) {
  ...
  return (
    <div className="screen" style={{ background: '#f8f1e8 url(/floral-bg.jpg) center/cover', padding: 'max(env(safe-area-inset-top, 20px), 20px) 20px' }}>
      ...
      <div className="glass-card" style={{ marginTop: '40px', borderRadius: '34px', padding: '30px 20px', position: 'relative' }}>
        <div style={{ position: 'absolute', top: '-24px', left: '50%', transform: 'translateX(-50%)', width: '48px', height: '48px', borderRadius: '50%', background: 'linear-gradient(135deg, var(--gold), #a67c00)', color: '#fff', display: 'grid', placeItems: 'center', fontSize: '24px', boxShadow: '0 4px 10px rgba(0,0,0,0.1)' }}>
          ♡
        </div>
        ...
        <button onClick={() => { if(message.trim() && name.trim()) onSend(message, name) }} style={{ width: '100%', padding: '16px', borderRadius: '999px', background: 'linear-gradient(135deg, var(--gold), #a67c00)', color: '#fff', fontFamily: 'var(--font-poppins)', fontWeight: 'bold', fontSize: '14px', marginBottom: '12px' }}>
          Send Memory
        </button>
