      ...
      <div style={{ display: 'flex', overflowX: 'auto', padding: '10px 16px', gap: '8px', scrollbarWidth: 'none' }}>
        {tabs.map(t => (
          <button key={t} onClick={() => onChangeTab(t)} style={{
            ...
            background: tab === t ? 'var(--cream-text)' : 'transparent',
            color: tab === t ? '#fff' : 'var(--cream-sub)',
            border: tab === t ? 'none' : '1px solid var(--gold-border)',
          ...
          {memories.filter(m => m.type === 'voice').map(m => (
            <div key={m.id} style={{ background: '#fff', borderRadius: '20px', border: '1px solid var(--gold-border)', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px 0' }}>
                <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'linear-gradient(145deg, var(--gold), #1f4a7c)', display: 'grid', placeItems: 'center', flexShrink: 0 }}>
      ...
      {(tab === 'all' || tab === 'notes') && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', padding: '4px 12px' }}>
          {memories.filter(m => m.type === 'message').map(m => {
            ...
            return (
              <div key={m.id} style={{ background: '#fff', borderRadius: '20px', border: '1px solid var(--gold-border)', padding: '14px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px' }}>
                  <div style={{ width: '38px', height: '38px', borderRadius: '50%', background: 'linear-gradient(145deg, #ffffff, var(--cream-bg2))', border: '1px solid var(--gold-border)', display: 'grid', placeItems: 'center', color: 'var(--cream-sub)', fontWeight: '700', fontSize: '15px', flexShrink: 0 }}>{initial}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontFamily: 'var(--font-poppins)', fontWeight: '700', fontSize: '13px', color: 'var(--cream-text)' }}>{gName}</div>
                    <div style={{ fontFamily: 'var(--font-dm-sans)', fontSize: '11px', color: 'var(--cream-sub)' }}>{formatTime(m.uploaded_at)}</div>
                  </div>
                  <button onClick={() => onLike(m.id)} style={{ color: 'var(--gold)', fontSize: '18px' }}>♡</button>
                </div>
