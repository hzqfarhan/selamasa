...
export default function GalleryScreen({ slug, coupleName, memories, tab, onChangeTab, onLoadMore, hasMore, onLike, onBack, onCaptureClick, onNavChange, onPhotoClick }: GalleryProps) {
  ...
  return (
    <div className="screen" style={{ background: 'linear-gradient(180deg,#f4f9ff 0%,#d5e9ff 100%)', display: 'flex', flexDirection: 'column', paddingBottom: 'calc(90px + var(--safe-bot))' }}>

      {/* Sticky Header */}
      <div style={{ position: 'sticky', top: 0, zIndex: 50, padding: 'max(env(safe-area-inset-top,14px),14px) 20px 10px', background: 'rgba(244,249,255,0.9)', backdropFilter: 'blur(14px)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(74,144,226,0.12)' }}>
        <button onClick={onBack} style={{ width: '38px', height: '38px', borderRadius: '50%', background: 'rgba(255,255,255,0.8)', border: '1px solid rgba(74,144,226,0.2)', display: 'grid', placeItems: 'center' }}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M15 18L9 12L15 6" stroke="#4a6f94" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
        </button>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontFamily: 'var(--font-poppins)', fontWeight: '700', fontSize: '13px', letterSpacing: '0.2em', color: '#102a43' }}>SELA MASA</div>
          <div style={{ fontFamily: 'var(--font-poppins)', fontSize: '9px', letterSpacing: '0.12em', color: '#4a6f94' }}>CAPTURE. CHERISH. FOREVER.</div>
        </div>
        <button style={{ width: '38px', height: '38px', borderRadius: '50%', background: 'rgba(255,255,255,0.8)', border: '1px solid rgba(74,144,226,0.2)', display: 'grid', placeItems: 'center' }}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M20.84 4.61C20.32 4.1 19.71 3.7 19.04 3.43C18.37 3.16 17.66 3.03 16.94 3.03C16.22 3.03 15.51 3.17 14.84 3.44C14.17 3.71 13.56 4.11 13.04 4.62L12 5.66L10.96 4.62C9.91 3.57 8.48 2.97 6.98 2.97C5.48 2.97 4.04 3.57 2.99 4.62C1.94 5.67 1.34 7.1 1.34 8.6C1.34 10.1 1.94 11.54 2.99 12.59L12 21.6L21.01 12.59C21.52 12.07 21.92 11.46 22.19 10.79C22.46 10.12 22.59 9.41 22.59 8.69C22.59 7.97 22.45 7.26 22.18 6.59C21.91 5.92 21.51 5.31 21 4.79L20.84 4.61Z" stroke="#4a6f94" strokeWidth="1.7" /></svg>
        </button>
      </div>

      {/* Hero Card */}
      <div style={{ margin: '12px 16px', borderRadius: '28px', minHeight: '200px', background: 'linear-gradient(135deg, #101c2c, #05090e)', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, rgba(5,9,14,0.7) 0%, transparent 60%)' }} />
        <div style={{ position: 'absolute', inset: '20px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          <p style={{ fontFamily: 'var(--font-great-vibes)', color: 'var(--cream-gold)', fontSize: '22px', marginBottom: '8px' }}>Engagement Memories</p>
          ...
          <div style={{ marginTop: '16px', display: 'inline-flex', alignItems: 'center', gap: '6px', background: 'rgba(255,255,255,0.15)', backdropFilter: 'blur(8px)', borderRadius: '999px', padding: '6px 14px', border: '1px solid rgba(255,255,255,0.2)', alignSelf: 'flex-start' }}>
            <span style={{ color: 'var(--cream-gold)', fontSize: '12px' }}>✨</span>
      ...
      <div style={{ display: 'flex', overflowX: 'auto', padding: '10px 16px', gap: '8px', scrollbarWidth: 'none' }}>
        {tabs.map(t => (
          <button key={t} onClick={() => onChangeTab(t)} style={{
            ...
            background: tab === t ? '#2f78c4' : 'transparent',
            color: tab === t ? '#fff' : 'var(--cream-sub)',
            border: tab === t ? 'none' : '1px solid rgba(74,144,226,.3)',
      ...
      {(tab === 'all' || tab === 'photos' || tab === 'videos') && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '9px', padding: '4px 12px' }}>
          {memories.filter(m => {
            ...
            <div key={m.id} className="memory-card" onClick={() => onPhotoClick(idx)} style={{ aspectRatio: '1/1', background: '#c7e1fa', borderRadius: '16px', overflow: 'hidden', position: 'relative' }}>
              {m.type === 'photo' ? (
      ...
      {(tab === 'all' || tab === 'voice') && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', padding: '4px 12px' }}>
          {memories.filter(m => m.type === 'voice').map(m => (
            <div key={m.id} style={{ background: '#fff', borderRadius: '20px', border: '1px solid rgba(74,144,226,0.18)', overflow: 'hidden' }}>
              {/* Header row */}
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px 0' }}>
                <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'linear-gradient(145deg,#3a86d4,#1e4e8c)', display: 'grid', placeItems: 'center', flexShrink: 0 }}>
                ...
                <div style={{ fontFamily: 'var(--font-poppins)', fontSize: '9px', fontWeight: '600', color: 'var(--cream-gold)', letterSpacing: '0.1em' }}>AUDIO GUESTBOOK</div>
              </div>
              {/* Waveform player */}
              <div style={{ padding: '12px 16px 14px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: 'linear-gradient(145deg,#3a86d4,#1e4e8c)', display: 'grid', placeItems: 'center', flexShrink: 0 }}>
      ...
      {(tab === 'all' || tab === 'notes') && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', padding: '4px 12px' }}>
          {memories.filter(m => m.type === 'message').map(m => {
            ...
            return (
              <div key={m.id} style={{ background: '#fff', borderRadius: '20px', border: '1px solid rgba(74,144,226,0.18)', padding: '14px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px' }}>
                  <div style={{ width: '38px', height: '38px', borderRadius: '50%', background: 'linear-gradient(145deg,#c7e1fa,#9bc7f2)', display: 'grid', placeItems: 'center', color: '#2f78c4', fontWeight: '700', fontSize: '15px', flexShrink: 0 }}>{initial}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontFamily: 'var(--font-poppins)', fontWeight: '700', fontSize: '13px', color: '#17110b' }}>{gName}</div>
                    <div style={{ fontFamily: 'var(--font-dm-sans)', fontSize: '11px', color: 'var(--cream-sub)' }}>{formatTime(m.uploaded_at)}</div>
                  </div>
                  <button onClick={() => onLike(m.id)} style={{ color: 'rgba(74,144,226,0.7)', fontSize: '18px' }}>♡</button>
                </div>
                <p style={{ fontFamily: 'var(--font-dm-sans)', fontSize: '13px', color: '#102a43', lineHeight: 1.6 }}>{text}</p>
              </div>
            )
          })}
        </div>
