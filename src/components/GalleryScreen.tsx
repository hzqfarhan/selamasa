          ...
          if (m.type === 'photo' || m.type === 'boomerang' || m.type === 'video') {
            return (
              <div key={m.id} className="memory-card" onClick={() => onPhotoClick(idx)} style={{ aspectRatio: '1/1', background: '#ccc' }}>
                {m.type === 'photo' ? (
                  <img src={m.fileUrl ?? undefined} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                ) : (
                  <video src={m.fileUrl ?? undefined} className="album-video" muted loop playsInline style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                )}
                {m.type === 'boomerang' && (
            ...
            return (
              <div key={m.id} className="glass-card" style={{ padding: '16px', borderRadius: '20px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '12px', fontWeight: 'bold', color: 'var(--cream-text)' }}>{m.guestName ?? m.guest_name}</div>
                  <div style={{ fontSize: '10px', color: 'var(--cream-sub)' }}>Voice Memory</div>
                  <audio src={m.fileUrl ?? undefined} controls style={{ width: '100%', height: '24px', marginTop: '8px' }} />
                </div>
              </div>
            )
            ...
            return (
              <div key={m.id} style={{ background: '#fff', padding: '16px', borderRadius: '20px', border: '1px solid var(--gold-border)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                  <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'var(--cream-bg2)', display: 'grid', placeItems: 'center', color: 'var(--cream-sub)', fontWeight: 'bold' }}>{(m.guestName ?? m.guest_name).charAt(0).toUpperCase()}</div>
                  <div style={{ flex: 1, fontSize: '12px', fontWeight: 'bold', color: 'var(--cream-text)' }}>{m.guestName ?? m.guest_name}</div>
