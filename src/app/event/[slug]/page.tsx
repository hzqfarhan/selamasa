...
export default function EventPage() {
  ...
  return (
    <>
      ...
      {currentScreen === 'welcome' && <WelcomeScreen onStartCapture={() => setCurrentScreen('guestname')} onViewAlbum={() => { setCurrentScreen('gallery'); loadMemories(true); setGalleryKey(k => k + 1) }} onVoiceMemory={() => setCurrentScreen('voice')} onWriteNote={() => setCurrentScreen('note')} />}
      ...
      {currentScreen === 'gallery' && <GalleryScreen key={galleryKey} slug={slug as string} coupleName={coupleName} memories={memories} tab={tab} onChangeTab={changeTab} onLoadMore={() => loadMemories()} hasMore={hasMore} onLike={handleLike} onBack={() => setCurrentScreen('welcome')} onCaptureClick={() => guestName ? setCurrentScreen('camera') : setCurrentScreen('guestname')} onNavChange={handleNavChange} onPhotoClick={setSelectedPhotoIndex} />}
      ...
      {currentScreen === 'profile' && <ProfileScreen onNavChange={handleNavChange} onCaptureClick={() => guestName ? setCurrentScreen('camera') : setCurrentScreen('guestname')} />}
      {currentScreen === 'home' && <HomeScreen onNavChange={handleNavChange} onCaptureClick={() => guestName ? setCurrentScreen('camera') : setCurrentScreen('guestname')} onViewAlbum={() => { setCurrentScreen('gallery'); loadMemories(true); setGalleryKey(k => k + 1) }} onVoiceMemory={() => setCurrentScreen('voice')} onWriteNote={() => setCurrentScreen('note')} coupleName={coupleName} />}
